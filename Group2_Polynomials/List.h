...
// Constructor in struct {D_Node}
template<class T>
List<T>::D_Node::D_Node(const T& data) : data(data), next(NULL), prev(NULL) {}
...
ostream& operator << (ostream& out, const List<T>& li) {
    out << '[';
    typename List<T>::D_Node* p = li.head;
    while (p) {
        out << p->data;
        if (p != li.tail) { out << ", "; }
        p = p->next;
...
public:
    ...
    T& operator * ();  // Retruns the element at current iterator position (l-value).
    const T& operator * () const;  // Retruns the element at current iterator position (r-value).
...
typename List<T>::Iterator& List<T>::Iterator::operator ++ () {
    if (!current) { throw out_of_range("Iterator out of range: " + to_string(offset + 1)); }
    current = current->next;
    offset++;
    return *this;
}  // Time complexity: O(1)
...
template<class T>
typename List<T>::Iterator& List<T>::Iterator::operator -- () {
    if (current == parent->head) { throw out_of_range("Iterator out of range: " + to_string((int)offset - 1)); }
...
// Retruns the element at current iterator position (l-value).
template<class T>
...
// Retruns the element at current iterator position (r-value).
template<class T>
